...
def matrix_diag(t):
    ...
    diag_mask = rearrange(i_range, 'i -> i 1') == rearrange(j_range, 'j -> 1 j')  # mask with True in diag
...
class PatchDropout(nn.Module):
    ...
    def forward(self, x, force_keep_all = False):
        if not self.training or self.prob == 0. or force_keep_all:  # when eval or force keep all, return original tensor 
            return x

        b, n, _, device = *x.shape, x.device  # batch,64 

        batch_indices = torch.arange(b, device = device)
        batch_indices = rearrange(batch_indices, '... -> ... 1')  # b,1
        num_patches_keep = max(1, int(n * (1 - self.prob)))  # 64*(1-0.5)=32
        patch_indices_keep = torch.randn(b, n, device = device).topk(num_patches_keep, dim = -1).indices  # b,32

        return x[batch_indices, patch_indices_keep]  # 1,64,512 -> 1,32,512
...
class GEGLU(nn.Module):
    def forward(self, x):
        x, gate = x.chunk(2, dim = -1) # 1,seq,4096 -> 1,seq,2048  1,seq,2048
        return x * F.gelu(gate)  # 1,seq,2048
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 4, dropout = 0.):
        ...
        )  # 1,seq,512 -> 1,seq,4096 -> 1,seq,2048 -> 1,seq,512
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask = None, rotary_pos_emb = None):
        ...
        q, k, v = self.to_qkv(x).chunk(3, dim = -1)  # 1,seq,512  1,seq,512  1,seq,512
        ...
        if exists(mask):
            ...
            sim = sim.masked_fill(~mask, mask_value)  # put mask_value to sim where elem=0 

        if self.causal:
            i, j = sim.shape[-2:]
            causal_mask = torch.ones((i, j), dtype = torch.bool, device = device).triu(j - i + 1)
            sim = sim.masked_fill(causal_mask, mask_value)  # only preserve the tri-up
        ...
        out = einsum('b h i j, b h j d -> b h i d', attn, v)  # 1,8,seq,seq * 1,8,seq,64 -> 1,8,seq,64
        out = rearrange(out, 'b h n d -> b n (h d)')  # 1,8,seq,64 -> 1,seq,512
        return self.to_out(out)  # 1,seq,512 -> 1,seq,512
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        x = self.norm_in(x)  # 1,seq,512 -> 1,seq,512

        for attn, ff in self.layers:
            attn, ff = map(checkpoint_fn, (attn, ff))

            x = attn(x, mask, rotary_pos_emb) + x  # 1,seq,512 -> 1,seq,512
            x = ff(x) + x  # 1,seq,512 -> 1,seq,512

        return self.norm_out(x)  # 1,seq,512 -> 1,seq,512
...
class TextTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.token_emb = nn.Embedding(num_tokens, dim)  # 10000,512
    ...
    def forward(self, x, mask = None):
        b, n, device = *x.shape, x.device  # batch, seq

        x = self.token_emb(x)  # 1,seq,512

        if exists(self.abs_pos_emb):
            pos_emb = self.abs_pos_emb(torch.arange(n, device = device))  # seq,512
            x = x + rearrange(pos_emb, 'n d -> 1 n d')  # 1,seq,512
        ...
        if exists(self.cls_token):
            cls_tokens = repeat(self.cls_token, 'd -> b 1 d', b = b)  # 512 -> 1,1,512
            x = torch.cat((cls_tokens, x), dim = 1)  # 1,(1+seq),512

            if exists(mask):
                mask = F.pad(mask, (1, 0), value = True)  # pad cls token (left pad one, right not pad)

        out = self.transformer(x, mask = mask, rotary_pos_emb = rotary_pos_emb)  # 1,seq[+1],512 -> 1,seq[+1],512
        return out
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        num_patches = (image_size // patch_size) ** 2  # (256/32)^2=64
        patch_dim = channels * patch_size ** 2  # 3*32*32=3072

        self.to_tokens = nn.Sequential(
            Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1 = patch_size, p2 = patch_size),
            nn.Linear(patch_dim, dim)
        )  # 1,3,256,256 -> 1,64,3072 -> 1,64,512

        self.pos_emb = nn.Embedding(num_patches, dim) # 64,512
    ...
    def forward(
        ...
        x = self.to_tokens(x)  # 1,3,256,256 -> 1,64,3072 -> 1,64,512
        b, n, _ = x.shape

        pos_emb = self.pos_emb(torch.arange(n, device = device))  # 64,512
        x = x + rearrange(pos_emb, 'n d -> 1 n d')   # 1,64,512

        x = self.patch_dropout(x, force_keep_all = keep_all_patches)  # 1,32,512

        out = self.transformer(x)  # 1,32,512

        cls_tokens = self.to_cls_tokens(out)  # 1,32,512 -> 1,512 -> 1,512 -> 1,1,512
        return torch.cat((cls_tokens, out), dim = 1)  # 1,33,512
        ...
        **kwargs
        ...
        if exists(text_encoder):
            ...
            self.text_transformer = TextTransformer(
                dim = dim_text,
                num_tokens = num_text_tokens + (1 if use_mlm else 0),   # add token represent [MASK]
...
if __name__ == '__main__':
    ...
    clip = CLIP(
        ...
        use_mlm = True,                        # use masked language learning (MLM) on text (DeCLIP)
